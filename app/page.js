...
const signup = () => {
  ...
  const registerMutation = async ({ name, email, password }) => {
    const response = await axios.post(
      "https://iseea.onrender.com/api/v1/auth/register",
      { name, email, password },
      {
        headers: {
        ...
        <main className="flex items-center justify-center px-8 py-8 sm:px-6 lg:col-span-7 lg:px-16 lg:py-12 xl:col-span-6">
          <div className="max-w-2xl lg:max-w-4xl">
            ...
            >
              ...
              {!passwordMatch && <ErrorAlert message={"Passwords do not match, Try again"} /> }
              {isSuccessful && (
                <SuccessAlert
                  message={"Account has been successfully created for"}
                  name={Name}
                />
              )}
            </form>
